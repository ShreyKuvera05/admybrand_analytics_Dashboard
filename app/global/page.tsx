...
export default function GlobalPage() {
  const globalMetrics = [
    ...
    { title: "Top Region", value: "North America", change: "38%", icon: MapPin },
    { title: "Global Growth", value: "+24.5%", change: "vs last month", icon: TrendingUp },
    { title: "International Users", value: "156K", change: "+12.8%", icon: Users },
  ]

  const topCountries = [
    { country: "United States", users: "45,231", percentage: "38.2%", flag: "🇺🇸" },
    { country: "United Kingdom", users: "23,456", percentage: "19.8%", flag: "🇬🇧" },
    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
      <div className="mb-6 sm:mb-8">
        ...
        <p className="text-gray-600 dark:text-slate-400 mt-2 text-sm sm:text-base">
          Worldwide insights and geographic performance metrics with interactive mapping
        </p>
      </div>
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-green-600 dark:text-green-400 text-sm">{metric.change}</p>
                </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              {topCountries.map((country, index) => (
                ...
                >
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      <p className="text-gray-900 dark:text-white font-medium">{country.country}</p>
                      <p className="text-gray-600 dark:text-gray-400 text-sm">{country.users} users</p>
                    </div>
                  </div>
                  <span className="text-blue-600 dark:text-blue-400 font-semibold">{country.percentage}</span>
                </div>
              ))}
            </div>
          </CardContent>
        ...
        <Card className="bg-white/80 dark:bg-black/20 backdrop-blur-xl border-gray-200 dark:border-white/10 shadow-light-lg dark:shadow-none">
          <CardHeader>
            <CardTitle className="text-gray-900 dark:text-white">Active by Timezone</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
              {timeZones.map((tz, index) => (
                ...
                >
                  <div>
                    <p className="text-gray-900 dark:text-white font-medium">{tz.zone}</p>
                    <p className="text-gray-600 dark:text-gray-400 text-sm">Peak: {tz.peak}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-green-600 dark:text-green-400 font-semibold">{tz.active}</p>
                    <p className="text-gray-600 dark:text-gray-400 text-xs">active now</p>
                  </div>
                </div>
              ))}
            </div>
